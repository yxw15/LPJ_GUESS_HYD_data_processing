...
# Target: VIS (Visp) station coordinates
target_lat <- 46.3029
target_lon <- 7.842958
target_alt <- 639.0
...
# Map descriptions to parameter shortnames (needed for availability check)
...
# Find stations that have at least some of the wanted parameters
stations_with_data <- inventory %>%
  filter(parameter_shortname %in% wanted_params$parameter_shortname) %>%
  pull(station_abbr) %>%
  unique()

# Find nearest stations with data, ensuring VIS is always included
vis_station <- stations %>% filter(station_abbr == "VIS")

nearest_stations_all <- stations %>%
  filter(station_abbr %in% stations_with_data,
         !is.na(station_coordinates_wgs84_lat), !is.na(station_coordinates_wgs84_lon)) %>%
  mutate(
...
# Ensure VIS is included
if(!any(nearest_stations_all$station_abbr == "VIS")) {
  nearest_stations_all <- bind_rows(vis_station %>% mutate(dist_km = 0, alt_diff_m = 0), nearest_stations_all)
}

nearest_10_stations <- nearest_stations_all %>%
  slice_head(n = 10) %>%
  select(station_abbr, station_name, station_height_masl,
         station_coordinates_wgs84_lat, station_coordinates_wgs84_lon,
         dist_km, alt_diff_m)

cat("\nSelected stations (VIS + 9 nearest with data):\n")
...
# Build date-range label for filenames
date_label <- paste0(gsub("-", "", define_start_date), "_to_", gsub("-", "", define_end_date))

# Get the maximum row count across all stations
station_row_counts <- combined_filled_data %>%
  group_by(primary_station_abbr) %>%
  summarise(row_count = n(), .groups = "drop")

max_rows <- max(station_row_counts$row_count)

cat("\nStation row counts:\n")
print(station_row_counts %>% arrange(desc(row_count)))

# Keep stations with complete (or near-complete) time series:
#   - Always keep VIS
#   - Keep stations with >= 95% of the maximum row count
min_required_rows <- floor(max_rows * 0.95)

stations_to_keep <- station_row_counts %>%
  filter(row_count >= min_required_rows | primary_station_abbr == "VIS") %>%
  pull(primary_station_abbr)

daily_filtered <- combined_filled_data %>%
  filter(primary_station_abbr %in% stations_to_keep)

cat("\nKeeping", length(stations_to_keep), "stations with >= 95% completeness:\n")
cat(paste(stations_to_keep, collapse = ", "), "\n")
...
filtered_path <- file.path(out_dir, paste0("all_filtered_", date_label, ".csv"))
write.csv(daily_filtered, filtered_path, row.names = FALSE)
...
cat("\nSelected columns:\n")
print(colnames(daily_csv_selected))
cat("\nPreview:\n")
print(head(daily_csv_selected))
cat("\nStructure:\n")
str(daily_csv_selected)

# Save the renamed/selected dataset
selected_path <- file.path(out_dir, paste0("all_selected_", date_label, ".csv"))
write.csv(daily_csv_selected, selected_path, row.names = FALSE)
cat("Saved:", selected_path, "\n")

# Save VIS-specific file
vis_data <- daily_csv_selected %>% filter(station_abbr == "VIS")
vis_path <- file.path(out_dir, paste0("VIS_station_", date_label, ".csv"))
write.csv(vis_data, vis_path, row.names = FALSE)
cat("Saved VIS station file:", vis_path, "\n")
cat("  VIS rows:", nrow(vis_data), "\n")
...
# ---- Save README ----
readme_path <- file.path(getwd(), "Data/MeteoSwiss_station/README_download_nearest_station_ten.md")
...
all_station <- read.csv("Data/MeteoSwiss_station/all_10_stations_filled_19910101_to_20251231.csv")

# Assuming your dataframe is named 'all_station'
station_counts <- all_station %>%
...
# Filter the dataframe to keep only stations with >= 12784 rows
all_station_filtered <- all_station %>%
  group_by(station_abbr) %>%
  filter(n() >= 12784) %>%
  ungroup()

# Verify the result by printing the new row counts
station_counts_new <- all_station_filtered %>%
  group_by(station_abbr) %>%
  summarise(n_rows = n(), .groups = "drop")

print(station_counts_new)


# Define the output file path
output_file <- "Data/MeteoSwiss_station/all_VIS_selected_meteo.csv"

# Write the filtered dataframe to a CSV file
# row.names = FALSE ensures we don't include an extra column of row numbers
write.csv(all_station_filtered, output_file, row.names = FALSE)

# Print a confirmation message
cat("Successfully saved the filtered dataset with", nrow(all_station_filtered), 
    "rows to:", output_file, "\n")
