...
# ==============================================================================
mort_df   <- read_csv("lpj_guess/lpj_guess_twd/lpj_control_drought_mort_kappa_annual_full.csv")
carbon_df <- read_csv("lpj_guess/lpj_guess_twd/lpj_control_drought_yearly_carbon_productivity_full.csv")

master_annual_wide <- mort_df %>%
  select(-any_of("date")) %>%
  full_join(carbon_df, by = c("treatment", "species", "year"))

# Calculate dynamic range for plots
min_yr <- min(master_annual_wide$year, na.rm = TRUE)
...
for(i in seq_len(nrow(var_registry))) {
  ...
  p <- ggplot(plot_ready_df, aes(x = year, y = .data[[target_var]], color = species)) +
    ...
    scale_x_continuous(breaks = seq(min_yr, max_yr, by = max(1, floor((max_yr - min_yr)/6)))) +
