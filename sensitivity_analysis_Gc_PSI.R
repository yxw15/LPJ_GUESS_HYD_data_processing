...
# Evaluates kr_max, ks_max, kl_max, isohydricity for 4 species × 2 conditions
#
# Usage (after all sensitivity runs complete and outputs are copied):
#   Rscript sensitivity_analysis_Gc_PSI.R
#
# Output:
#   sensitivity_results/
...
#     parameter_sensitivity_Gc.pdf    — heatmaps & ranking plots (Gc)
#     parameter_sensitivity_PSI.pdf   — heatmaps & ranking plots (ΨL)
#     combined_score_summary.csv      — combined Gc+ΨL score ranking
# ==========================================================================
...
OUTPUT_DIR     <- file.path(BASE_DIR, "Figures/lpj_guess_stem_storage/sensitivity")
...
# ==========================================================================
# All four species share the same OAT test values.
# Only baseline values are species-specific (from .ins files).

parameter_bases <- list(
  Oak    = list(kr_base = 1.8,  ks_base = 2.1,  kl_base = 10,   iso_base = 0.4),
  Beech  = list(kr_base = 0.8,  ks_base = 1.0,  kl_base = 25,   iso_base = 0.6),
  Spruce = list(kr_base = 0.5,  ks_base = 0.7,  kl_base = 33.1, iso_base = 0.7),
  Pine   = list(kr_base = 0.6,  ks_base = 0.8,  kl_base = 12.5, iso_base = 0.7)
)

# Common OAT test values (same for all species, KR/KS step 0.2, ISO step 0.1)
oat_kr  <- seq(0.1, 3.5, 0.2)
oat_ks  <- seq(0.1, 4.0, 0.2)
oat_kl  <- seq(5, 35, 2)
oat_iso <- seq(-0.3, 0.8, 0.1)
...
for (sp in names(parameter_bases)) {
  ...
  for (cond in c("control", "drought")) {
    ...
    fmt_tag <- function(x) {
      x <- round(x, digits = 8)  # eliminate floating-point artifacts (e.g. 5.55e-17 → 0)
      ifelse(x == round(x), sprintf("%.1f", x), as.character(x))
    }

    combos <- list()

    # 1) Baseline run (species-specific values)
    combos[[1]] <- list(
      tag = "baseline",
      kr_max = bases$kr_base,
      ks_max = bases$ks_base,
      kl_max = bases$kl_base,
      isohydricity = bases$iso_base
    )

    combo_idx <- 2

    # 2) kr_max OAT (common values, other params at species base)
    for (kr in oat_kr) {
      combos[[combo_idx]] <- list(
        tag = paste0("kr", fmt_tag(kr)),
        kr_max = kr, ks_max = bases$ks_base,
        kl_max = bases$kl_base, isohydricity = bases$iso_base
      )
    ...
    for (ks in oat_ks) {
      combos[[combo_idx]] <- list(
        tag = paste0("ks", fmt_tag(ks)),
        kr_max = bases$kr_base, ks_max = ks,
        kl_max = bases$kl_base, isohydricity = bases$iso_base
      )
      combo_idx <- combo_idx + 1
    }
    ...
    for (kl in oat_kl) {
      combos[[combo_idx]] <- list(
        ...
        kr_max = bases$kr_base, ks_max = bases$ks_base,
        kl_max = kl, isohydricity = bases$iso_base
      )
      combo_idx <- combo_idx + 1
    }
    ...
    for (iso in oat_iso) {
      combos[[combo_idx]] <- list(
        tag = paste0("iso", fmt_tag(iso)),
        kr_max = bases$kr_base, ks_max = bases$ks_base,
        kl_max = bases$kl_base, isohydricity = iso
      )
      combo_idx <- combo_idx + 1
    }
...
if (nrow(metrics_gc) == 0 && nrow(metrics_psi) == 0) {
  ...
  cat("  ", RESULTS_BASE, "/{Species}_{condition}_{tag}/\n")
...
analyze_oat <- function(metrics_df, var_name) {
  ...
  for (sp in unique(metrics_df$species)) {
    for (cond in unique(metrics_df$condition)) {
      ...
      for (param in c("kr_max", "ks_max", "kl_max", "isohydricity")) {
        ...
        # Find best value based on composite score (KGE-weighted)
        # Composite = 0.4*R² + 0.3*KGE + 0.3*(1 - |bias|/range)
        # (higher is better; KGE scaled from possibly negative to positive range)
        best <- oat_runs %>%
          mutate(
...
if (nrow(metrics_gc) > 0) {
  ...
  ggsave(file.path(OUTPUT_DIR, "parameter_sensitivity_Gc_heatmap.png"),
         p_gc_heat, width = 14, height = 8, dpi = 300)
  ...
  ggsave(file.path(OUTPUT_DIR, "parameter_response_curve_Gc.png"),
         p_gc_resp, width = 14, height = 9, dpi = 300)
}
...
if (nrow(metrics_psi) > 0) {
  ...
  ggsave(file.path(OUTPUT_DIR, "parameter_sensitivity_PSI_heatmap.png"),
         p_psi_heat, width = 14, height = 8, dpi = 300)
  ...
  ggsave(file.path(OUTPUT_DIR, "parameter_response_curve_PSI.png"),
         p_psi_resp, width = 14, height = 9, dpi = 300)
}
...
if (exists("combined_scores") && nrow(combined_scores) > 0) {
  for (i in seq_len(nrow(combined_scores))) {
    r <- combined_scores[i, ]
    cat(sprintf("  %-8s %-10s: kr_max=%-5.1f ks_max=%-5.1f kl_max=%-5.1f isohydricity=%-5.1f (score=%.3f)\n",
                r$species, r$condition,
                r$kr_max, r$ks_max, r$kl_max, r$isohydricity, r$combined_score))
  }
} else {
  ...
  if (nrow(best_gc) > 0) {
    ...
    for (i in seq_len(nrow(best_gc))) {
      r <- best_gc[i, ]
      cat(sprintf("  %-8s %-10s: kr_max=%-5.1f ks_max=%-5.1f kl_max=%-5.1f isohydricity=%-5.1f\n",
                  r$species, r$condition,
...
combined_best_parameters <- read.csv("Figures/lpj_guess_stem_storage/sensitivity/combined_best_parameters.csv")
