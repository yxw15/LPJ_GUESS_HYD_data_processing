...
pick_numeric_col <- function(df, wanted = NULL, file_for_msg = "") {
  ...
  if (length(numeric_candidates) == 0) {
    stop(
      "No numeric data column found in file: ", file_for_msg, "\n",
      "Available columns: ", paste(names(df), collapse = ", ")
    )
  }
  
  message(
    "Note: column '", wanted, "' not found in ", basename(file_for_msg),
...
species_results <- pmap(
  ...
  function(species, colname) {
    ...
    joined <- inner_join(mort, kappa, by = c("Date", "Species"))
...
df_mort_kappa <- bind_rows(species_results) %>%
  filter(is.finite(mort_cav_day), is.finite(kappa_s_min)) %>%
  mutate(Species = factor(Species, levels = species_map$species)) %>%
  apply_year_filter()

if (nrow(df_mort_kappa) == 0) {
  stop(
    "Merged dataset exists but has 0 rows after filtering.\n",
    "Check year_min/year_max and whether mort_cav_day / kappa_s_min contain finite values."
  )
}
...
write.csv(
  df_mort_kappa,
  file.path(out_dir, "mort_cav_day_vs_kappa_s_min_merged_daily.csv"),
  row.names = FALSE
)
...
# RAW scatter plots
# ---------------------------
p_mort_kappa_raw_all <- ggplot(
  df_mort_kappa,
  aes(x = kappa_s_min, y = mort_cav_day, color = Species)
) +
  geom_point(alpha = 0.25, size = 1) +
  scale_color_manual(values = cb_palette, drop = FALSE) +
  labs(
    title = "Hydraulic mortality vs maximum cavitation fraction (RAW)",
    x = "Maximum cavitation fraction (kappa_s_min)",
...
ggsave(
  file.path(out_dir, "mort_cav_day_vs_kappa_s_min_scatter_RAW_ALL.png"),
  p_mort_kappa_raw_all,
  width = 10,
  height = 6,
  dpi = 300
)

p_mort_kappa_raw_facet <- ggplot(
  df_mort_kappa,
  aes(x = kappa_s_min, y = mort_cav_day, color = Species)
) +
  geom_point(alpha = 0.25, size = 1) +
  ...
  scale_color_manual(values = cb_palette, drop = FALSE) +
  labs(
    title = "Hydraulic mortality vs maximum cavitation fraction (RAW) — species panels",
    x = "Maximum cavitation fraction (kappa_s_min)",
    y = "Hydraulic mortality (mort_cav_day)"
  ) +
  theme_clean() +
...
ggsave(
  file.path(out_dir, "mort_cav_day_vs_kappa_s_min_scatter_RAW_facet.png"),
  p_mort_kappa_raw_facet,
  width = 11,
  height = 6,
  dpi = 300
)
...
# Bin data
# ---------------------------
b_mort_kappa <- bin_by_x(
  df = df_mort_kappa,
  x_col = "kappa_s_min",
  y_col = "mort_cav_day",
  bin_width = bin_width_kappa,
  min_n = min_n_bin
)

if (nrow(b_mort_kappa) == 0) {
...
write.csv(
  b_mort_kappa,
  file.path(out_dir, "mort_cav_day_vs_kappa_s_min_binned_0p01.csv"),
  row.names = FALSE
)
...
# ---------------------------
p_mort_kappa_binned_all <- ggplot() +
  geom_point(
    data = df_mort_kappa,
    aes(x = kappa_s_min, y = mort_cav_day),
    color = "grey60",
    alpha = 0.08,
    size = 0.5
  ) +
  ...
  labs(
    title = "Hydraulic mortality vs maximum cavitation fraction (binned x = 0.01) — ALL species",
    x = "Maximum cavitation fraction (kappa_s_min)",
    y = "Hydraulic mortality (mort_cav_day)",
    color = NULL,
    fill = NULL
...
ggsave(
  file.path(out_dir, "mort_cav_day_vs_kappa_s_min_binned0.01_smooth_ALL.png"),
  p_mort_kappa_binned_all,
  width = 11,
  height = 6,
  dpi = 300
)

p_mort_kappa_binned_facet <- ggplot() +
  geom_point(
    data = df_mort_kappa,
    aes(x = kappa_s_min, y = mort_cav_day),
    color = "grey60",
  ...
  scale_fill_manual(values = cb_palette, drop = FALSE) +
  labs(
    title = "Hydraulic mortality vs maximum cavitation fraction (binned x = 0.01) — species panels",
    x = "Maximum cavitation fraction (kappa_s_min)",
    y = "Hydraulic mortality (mort_cav_day)"
  ) +
  theme_clean() +
...
ggsave(
  file.path(out_dir, "mort_cav_day_vs_kappa_s_min_binned0.01_smooth_facet.png"),
  p_mort_kappa_binned_facet,
  width = 11,
  height = 6,
  dpi = 300
)
...
# ---------------------------
cat("\nSaved files to:\n", out_dir, "\n")
cat("\nNumber of raw rows:", nrow(df_mort_kappa), "\n")
cat("Number of binned rows:", nrow(b_mort_kappa), "\n")

# ---------------------------
# Show in RStudio
# ---------------------------
print(p_mort_kappa_raw_all)
print(p_mort_kappa_raw_facet)
print(p_mort_kappa_binned_all)
print(p_mort_kappa_binned_facet)