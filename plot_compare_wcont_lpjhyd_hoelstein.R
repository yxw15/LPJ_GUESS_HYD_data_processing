...
output_dir = "Figures/lpj_guess_hyd"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# ==========================================
# 1. read and process process model data (mwcont)
# ==========================================
file_list = list.files(
  path = "results_lpj/results_hoelstein_hyd", 
  pattern = "mwcont_(upper|lower)\\.out$", 
  recursive = TRUE, 
  full.names = TRUE 
)
...
mwcont_all_combined = file_list %>% 
  map_df(function(file_path) {
    ...
    # path_parts map out to: 
    # [1] "results_lpj" [2] "results_hoelstein_twd" [3] treatment [4] species [5] file
    current_treatment = tolower(path_parts[3])
    current_species   = tolower(path_parts[4]) 
    current_layer     = ifelse(str_detect(file_path, "upper"), "upper", "lower")
    ...
    read.table(file_path, header = TRUE) %>%
      ...
      select(`year-month`, wcont_value, species, treatment, layer)
  })
...
# plot 1: modelled water content (mwcont)
p1 = ggplot(mwcont_filtered, aes(x = plot_date, y = wcont_value, color = layer)) +
...
mw_to_merge = mwcont_filtered %>%
  ...
  mutate(
    ...
    source_layer = case_when(
      layer == "upper" ~ "model: upper layer",
      layer == "lower" ~ "model: lower layer"
    )
  ) %>%
...
# 5. define custom distinct color hex codes
custom_colors = c(
  "model: upper layer" = "#e66101", 
  "model: lower layer" = "#b2182b", 
...
# ==========================================
# 7. plot 5: distribution of values for months 6-9 only
# ==========================================
