...
daily_filtered <- daily_filtered %>%
  ...
  select(
    ...
    temp_daily_mean = Air.temperature.2.m.above.ground..daily.mean,
    
    radiation_daily_mean = Global.radiation..daily.mean,
    relhum_daily_mean    = Relative.air.humidity.2.m.above.ground..daily.mean,
    
    wind_daily_mean = Wind.speed.scalar..daily.mean.in.m.s,
    precip_daily    = Precipitation..daily.total.0.UTC...0.UTC
  )
